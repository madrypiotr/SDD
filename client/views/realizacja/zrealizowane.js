Template.realizacjaTab2.helpers({
    'settings': function () {
        return {
            ...
            fields: [{
                key: 'dataRealizacji',
                label: TAPi18n.__('txv.START_OF_THE_IMPLEMENTATION'),
                tmpl: Template.dataRealizKwestia
            }, {
                key: 'kwestiaNazwa',
                label: TAPi18n.__('glob.NameIssue'),
                tmpl: Template.nazwaKwestiLink
            }, {
                key: 'idTemat',
                label: TAPi18n.__('glob.Subject'),
                tmpl: Template.tematKwestia
            }, {
                key: 'raporty',
                label: TAPi18n.__('txv.REPORT'),
    ...
    ZrealizowaneListCount: function () {
        return Kwestia.find({czyAktywny: true, status: {$in: [KWESTIA_STATUS.ZREALIZOWANA]}}).count();
    }
});
        ...
        if (this.idZespolRealizacyjny) {
            ...
            Meteor.call('serverGetFullName', realizationTeam, function (error, ret) {
                if (!error) {
                    ...
                    var docDefinition = {
                        content: [{
                            text: TAPi18n.__('txv.DATA') + ' ' + moment(this.dataRealizacji).format('DD.MM.YYYY').toString() + ' ' + TAPi18n.__('txv.R'),
                            style: 'uchwalaTop'
                        }, {
            ...
            var docDefinition = {
                content: [{
                    text: TAPi18n.__('txv.DN') + ' ' + moment(this.dataRealizacji).format('DD.MM.YYYY').toString() + ' ' + TAPi18n.__('txv.R'),
                    style: 'uchwalaTop'
                }, {
                    text: globalParameters.nazwaOrganizacji + '\n' +
                    globalParameters.terytorium + '\n' +
